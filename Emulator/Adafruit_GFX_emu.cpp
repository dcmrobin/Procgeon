...
// Complete 5x8 bitmap font for u8g2_font_profont10_mf (ASCII 32–126)
...
size_t Adafruit_GFX::print(const std::string &str) {
    size_t count = 0;
    //const uint8_t* activeFont = font == profont10_font ? font : builtin_font;#
    const uint8_t* activeFont = font == profont10_font ? font : builtin_font;
    int fontWidth = 5;
    int fontHeight = 8;
    // If using profont10, adjust width/height if needed
    if (font == profont10_font) {
        fontWidth = 5; // profont10 is 5x8 in this sample
        fontHeight = 8;
    }
    for (char c : str) {
