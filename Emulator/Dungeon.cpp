...
void generateDungeon() {
    ...
    const int maxRooms = rand() % 11 + 15;  // 15-25 rooms
    ...
    // Starting room
    int startRoomWidth = rand() % (maxRoomSize - minRoomSize + 1) + minRoomSize;
    int startRoomHeight = rand() % (maxRoomSize - minRoomSize + 1) + minRoomSize;
    ...
    for (int i = 1; i < maxRooms; i++) {
        ...
        int roomX = rand() % (mapWidth - roomWidth - 6) + 3;
        int roomY = rand() % (mapHeight - roomHeight - 6) + 3;
    ...
    // Spawn chests
    int chestCount = rand() % 3 + 2; // 2-4 chests
    for (int i = 0; i < chestCount; i++) {
        int roomIndex = rand() % roomCount;
        Room &room = rooms[roomIndex];
        int chestX = room.x + rand() % (room.width - 2) + 1;
        int chestY = room.y + rand() % (room.height - 2) + 1;
            ...
            if (dungeonMap[y][x] == Wall) {
                ...
                for (int nx = -1; nx < 2; nx++) {
                    for (int ny = -1; ny < 2; ny++) {
                        ...
                        if (y + ny >= 0 && y + ny < mapHeight && 
                            x + nx >= 0 && x + nx < mapWidth && 
                            dungeonMap[y + ny][x + nx] == Wall) {
                            neighbors += 1;
                        }
                    }
                }
    ...
    if (dungeon > levelOfDamselDeath + 3) {
        ...
        do {
            damselRoomX = rand() % (mapWidth - damselRoomWidth - 6) + 3;
            damselRoomY = rand() % (mapHeight - damselRoomHeight - 6) + 3;
        ...
        for (int y = damselRoomY; y < damselRoomY + damselRoomHeight; y++) {
            for (int x = damselRoomX; x < damselRoomX + damselRoomWidth; x++) {
                if ((y == damselRoomY || y == damselRoomY + damselRoomHeight - 1) && 
                    x >= damselRoomX && x < damselRoomX + damselRoomWidth) {
                    dungeonMap[y][x] = Bars;
                } else {
                    dungeonMap[y][x] = Floor;
                }
    ...
    dungeonMap[playerStartY][playerStartX] = Floor;
    ...
    // Equipment
    int equipmentCount = rand() % 3 + 1;
    for (int i = 0; i < equipmentCount; i++) {
        int roomIndex = rand() % (roomCount - 2) + 1;
        Room &room = rooms[roomIndex];
        int itemX = room.x + rand() % (room.width - 2) + 1;
        int itemY = room.y + rand() % (room.height - 2) + 1;
        if (dungeonMap[itemY][itemX] == Floor) {
            dungeonMap[itemY][itemX] = rand() % 100 > 50 ? ArmorTile : RingTile;
        }
    }
}
    ...
    for (int i = 0; i < maxEnemies; i++) {
        while (true) {
            ...
            if (dungeonMap[ey][ex] == Floor && 
                sqrt(pow(playerX - ex, 2) + pow(playerY - ey, 2)) >= 10) {
                if (rand() % 30 <= 24) {
                    enemies[i] = { (float)ex, (float)ey, 20, false, 0.05f, "blob", 20, 5, false, 0, 0 };
                } else {
                    enemies[i] = { (float)ex, (float)ey, 10, false, 0.11f, "teleporter", 20, 0, false, 0, 0 };
                }
                break;
            }
        }
...
void updateScrolling(int viewportWidth, int viewportHeight, float scrollSpeed, float& offsetX, float& offsetY) {
    ...
    targetOffsetX = std::max(0.0f, std::min((float)(mapWidth - viewportWidth), targetOffsetX));
    targetOffsetY = std::max(0.0f, std::min((float)(mapHeight - viewportHeight), targetOffsetY));
